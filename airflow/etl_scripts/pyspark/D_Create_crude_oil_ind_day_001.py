...
HIVE_CONFIG = {
    "source_schema": "oil_raw",
    "source_table": "crude_oil_price_day",
...
def main(dwh_module_home, N_month, update_N_months_from):
    ...
    with BaseSparkSession(SPARK_CONFIG, base_spark_app_name) as spark_session:
        ...
        oil_raw_df = spark_session.run_sql(query)

        # Select distinct symbols. (e.g., CL=F)
        crude_oil_symbol_df = spark_session.run_sql(
        ...
        for row in crude_oil_symbol_df.select(crude_oil_symbol_df.id).collect():
            ...
            quotes = get_quotes(pd_crude_oil_history_df, N_multiple)
