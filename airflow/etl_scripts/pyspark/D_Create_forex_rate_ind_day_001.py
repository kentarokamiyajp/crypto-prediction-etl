...
HIVE_CONFIG = {
    "source_schema": "forex_raw",
    "source_table": "forex_rate_day",
...
def main(dwh_module_home, N_month, update_N_months_from):
    ...
    with BaseSparkSession(SPARK_CONFIG, base_spark_app_name) as spark_session:
        ...
        forex_raw_df = spark_session.run_sql(query)

        # Select distinct symbols. (e.g., CL=F)
        forex_rate_symbol_df = spark_session.run_sql(
        ...
        for row in forex_rate_symbol_df.select(forex_rate_symbol_df.id).collect():
            ...
            quotes = get_quotes(pd_forex_rate_history_df, N_multiple)
            ...
            sp_history_with_indicators_df = sp_forex_rate_history_df.join(
                ...
                sp_forex_rate_history_df.dt == sp_all_indicaters_df.dt_,
                "outer",
            )
