...
HIVE_CONFIG = {
    "source_schema": "crypto_raw",
    "source_table": "candles_day",
    ...
    with BaseSparkSession(SPARK_CONFIG, base_spark_app_name) as spark_session:
        ...
        for row in crypto_symbol_df.select(crypto_symbol_df.id).collect():
            ...
            if row.id in exceptional_symbol:
                N_multiple = 1000.0
            quotes = get_quotes(pd_crypto_history_df, N_multiple)
        ...
        #############################################
        # 3. Insert the calculated indicator values to the hive mart WRK table
        #############################################
