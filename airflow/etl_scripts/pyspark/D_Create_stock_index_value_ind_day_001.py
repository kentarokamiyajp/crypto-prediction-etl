...
def main(dwh_module_home, N_month, update_N_months_from):
    ...
    with BaseSparkSession(SPARK_CONFIG, base_spark_app_name) as spark_session:
        ...
        stock_index_df = spark_session.run_sql(query)

        # Select distinct symbols. (e.g., CL=F)
        stock_index_symbol_df = spark_session.run_sql(
        ...
        for row in stock_index_symbol_df.select(stock_index_symbol_df.id).collect():
            ...
            quotes = get_quotes(pd_stock_index_history_df, N_multiple)
