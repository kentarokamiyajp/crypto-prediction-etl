...
#############################################
target_schema = "stock_raw"
target_table = "stock_index_day"
query = f"select id, cast(dt as string) as dt, open, high, low, close, volume, \
    year, month, day from {target_schema}.{target_table}"
stock_index_value_raw_df = spark.sql(query)

# Select distinct symbols.
stock_index_value_symbol_df = spark.sql(f"select distinct id from {target_schema}.{target_table}")
...
final_results = None
for row in stock_index_value_symbol_df.select(stock_index_value_symbol_df.id).collect():
    print("Target Stock Index Symbol:", row.id)
    sp_stock_index_value_history_df = stock_index_value_raw_df.filter(stock_index_value_raw_df.id == row.id)
    pd_stock_index_value_history_df = sp_stock_index_value_history_df.toPandas()
    ...
    quotes = [
        ...
        for d, o, h, l, c, v in zip(
            pd_stock_index_value_history_df["dt"],
            pd_stock_index_value_history_df["open"],
            pd_stock_index_value_history_df["high"],
            pd_stock_index_value_history_df["low"],
            pd_stock_index_value_history_df["close"],
            pd_stock_index_value_history_df["volume"],
        )
    ]
    ...
    sp_history_with_indicators_df = sp_stock_index_value_history_df.join(
        sp_all_indicaters_df, sp_stock_index_value_history_df.dt == sp_all_indicaters_df.dt_, "outer"
    )
