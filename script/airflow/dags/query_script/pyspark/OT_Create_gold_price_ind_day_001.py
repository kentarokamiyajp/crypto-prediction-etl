...
#############################################
target_schema = "gold_raw"
target_table = "gold_price_day"
query = f"select id, cast(dt as string) as dt, open, high, low, close, volume, \
    year, month, day from {target_schema}.{target_table}"
gold_price_raw_df = spark.sql(query)

# Select distinct symbols.
gold_price_symbol_df = spark.sql(f"select distinct id from {target_schema}.{target_table}")
...
final_results = None
for row in gold_price_symbol_df.select(gold_price_symbol_df.id).collect():
    print("Target Gold Symbol:", row.id)
    sp_gold_price_history_df = gold_price_raw_df.filter(gold_price_raw_df.id == row.id)
    pd_gold_price_history_df = sp_gold_price_history_df.toPandas()
    ...
    quotes = [
        ...
        for d, o, h, l, c, v in zip(
            pd_gold_price_history_df["dt"],
            pd_gold_price_history_df["open"],
            pd_gold_price_history_df["high"],
            pd_gold_price_history_df["low"],
            pd_gold_price_history_df["close"],
            pd_gold_price_history_df["volume"],
        )
    ]
    ...
    sp_history_with_indicators_df = sp_gold_price_history_df.join(
        sp_all_indicaters_df, sp_gold_price_history_df.dt == sp_all_indicaters_df.dt_, "outer"
    )
