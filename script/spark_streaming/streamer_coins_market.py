...
value_df_coins_market = readDF_coins_market.select(from_json(col("value").cast("string"),schema).alias("value"))
exploded_df_coins_market = value_df_coins_market.selectExpr('explode(value.data) as data')

flattened_df_coins_market = exploded_df_coins_market \
    .withColumn('id',expr('data.id'))\
    ...
    .withColumn('market_cap_change_percentage_24h',expr('data.market_cap_change_percentage_24h'))\
...
def writeToCassandra_coins_market(writeDF, _):
    ...
    .mode('append')\
    .options(table="coins_markets", keyspace="crypto")\
    .save()

logger.info('{} INFO Upserted the {} data into Cassandra'.format(datetime.now(),KAFKA_TOPIC_coins_markets))
write_stream_coins_market = flattened_df_coins_market.writeStream \
    .foreachBatch(writeToCassandra_coins_market) \
    .outputMode("update") \
    .start() \
    .awaitTermination()
