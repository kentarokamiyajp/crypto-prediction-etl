...
value_df_coins_by_id = readDF_coins_by_id.select(from_json(col("value").cast("string"),schema).alias("value"))

flattened_df_coins_by_id = value_df_coins_by_id \
    .withColumn('id',value_df_coins_by_id['value']['id'])\
    ...
    .withColumn('block_time_in_minutes',value_df_coins_by_id['value']['block_time_in_minutes'])\
    ...
    .withColumn('sentiment_votes_up_percentage',value_df_coins_by_id['value']['sentiment_votes_up_percentage'])\
    .withColumn('sentiment_votes_down_percentage',value_df_coins_by_id['value']['sentiment_votes_down_percentage'])\
    ...
    .withColumn('public_interest_score',value_df_coins_by_id['value']['public_interest_score'])\
    ...
    .withColumn('public_interest_stats',value_df_coins_by_id['value']['public_interest_stats'])\
...
def writeToCassandra_coins_by_id(writeDF, _):
    ...
    .mode('append')\
    .options(table="coins_by_id", keyspace="crypto")\
    .save()

print('{} INFO Upserted the {} data into Cassandra'.format(datetime.now(),KAFKA_TOPIC_coins_by_id))
write_stream_coins_by_id = flattened_df_coins_by_id.writeStream \
    .foreachBatch(writeToCassandra_coins_by_id) \
    .outputMode("append") \
    .start() \
    .awaitTermination()
