...
consumer_id = "realtime_trading_dashboard_v5"
topic_id = "crypto.realtime_trading_bot_v5"
# group_id = os.environ.get("GROUP_ID")
group_id = "realtime_trading_dashboard_v5_" + datetime.now().strftime("%Y%m%d%H%M")
...
while True:
    ...
    if df.empty:
        ...
        if str(consumed_data["ts_send_utc"]) == str(df.iloc[-1]["ts_send_utc"]):
            df = df.iloc[:-1]
            
        df = pd.concat([df, new_row], ignore_index=True).sort_values(by=["ts_send_utc"])
        # print(df)

    save_time_diff = (datetime.now() - prev_save_time).total_seconds()
    if len(df) > 1 and save_time_diff > 60:
        with open(
