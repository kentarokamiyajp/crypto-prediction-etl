...
# dashboard title
st.title("Real-Time BTC Trading Dashboard (V5)")
...
while True:
    try:
        with open(
            "/Users/kamiken/git/crypto_prediction_dwh/script/kafka_consumers/data_for_rt_dashboard_v5.pkl",
            "rb",
        ) as f:
            df = pickle.load(f)
            df["bought_price"] = df["bought_price"].replace(regex=r"-99.9", value=None)
            df.loc[df["transaction_type"] != "bought", "bought_price"] = (
                None  # keep bought price only when the transaction value is "bought"
            )
    ...
    with placeholder.container():
        ...
        kpi01.metric(label="TS Candle UTC", value=str(df.iloc[-1]["ts_send_utc"]))
        ...
        with st.container():
            ...
            fig_price = px.line(data_frame=df.iloc[-5000:], y="close", x="ts_send_utc")

            fig_bought = px.line(
                data_frame=df.iloc[-5000:], y="bought_price", x="ts_send_utc", markers=True
            )
            ...
            fig_sold = px.line(
                data_frame=df.iloc[-5000:], y="sold_price", x="ts_send_utc", markers=True
            )
        ...
        with st.container():
            st.markdown("### MACD")\
            fig_macd = px.line(
                data_frame=df.iloc[-5000:],
                y=["macd", "macd_signal"],
                x="ts_send_utc",
                color_discrete_map={"macd": "#ff6c62", "macd_signal": "#62ff7a"},
            )
            st.write(fig_macd)
        ...
        with st.container():
            ...
            fig_stoch = px.line(
                ...
                y=["stoch_k", "stoch_d"],
                x="ts_send_utc",
                color_discrete_map={"stoch_k": "#ff6c62", "stoch_d": "#62ff7a"},
            )
            st.write(fig_stoch)
        ...
        with fig_col2:
            ...
            fig2 = px.line(data_frame=df, y="my_money", x="ts_send_utc")
            st.write(fig2)
        with fig_col3:
            today = df["dt_send_utc"].max()
            st.markdown(f"### Today's Profits ({today})")
            df_profits = df[df["dt_send_utc"] == today]
            fig3 = px.line(data_frame=df_profits, y="total_profit_in_a_day", x="ts_send_utc")
            st.write(fig3)
