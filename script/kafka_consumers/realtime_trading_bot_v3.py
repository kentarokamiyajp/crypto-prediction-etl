...
def consume_realtime_data(curr_date, curr_timestamp, consumer_id):
    ...
    group_id = "realtime_trading_bot_v3_" + datetime.now().strftime("%Y%m%d%H%M")
...
def produce_minute_data(producer, message, key):
    ...
    topic_name = "crypto.realtime_trading_bot_v3"
    num_partitions = 1
...
def trade_main(indicators_df):
    ...
    if len(TRADE_RESULT) % 10 == 0 and save_data:
        print("SAVING")
        TRADE_RESULT.to_csv(
            "/home/pyuser/git/crypto_prediction_dwh/script/kafka_consumers/trade_variables_v3.csv"
        )
