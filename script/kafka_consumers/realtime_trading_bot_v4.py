...
def consume_realtime_data(curr_date, curr_timestamp, consumer_id):
    ...
    group_id = "realtime_trading_bot_v4_" + datetime.now().strftime("%Y%m%d%H%M")
...
def produce_minute_data(producer, message, key):
    ...
    topic_name = "crypto.realtime_trading_bot_v4"
    num_partitions = 1
...
def trade_main(indicators_df):
    ...
    # Start Trading
        ...
        if stop_trading_today:
            ...
            if is_long_position:
                ...
                if should_sell or (should_sell == False and gain_is_negative_from_prev > 10 and current_gain < -0.05):
                    # sell
                    is_long_position = False
    ...
    if len(TRADE_RESULT) % 10 == 0 and save_data:
        print("SAVING")
        TRADE_RESULT.to_csv(
            "/home/pyuser/git/crypto_prediction_dwh/script/kafka_consumers/trade_variables_v4.csv"
        )
