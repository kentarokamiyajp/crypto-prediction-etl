...
def main():
    ...
    cass_ope = cassandra_operator.CassandraOperator(keyspace)
    insert_query = f"""
    INSERT INTO {table_name} (id,trade_id,takerSide,amount,quantity,price,createTime,ts_send,dt_create_utc,ts_create_utc,ts_insert_utc) \
        ...
        try:
            ...
            if curr_retry_cnt >= max_retry_cnt:
                consumer.logger.error(
                    "Kafka Consumer failed !!! Retry ({}/{})".format(curr_retry_cnt, max_retry_cnt)
                )
