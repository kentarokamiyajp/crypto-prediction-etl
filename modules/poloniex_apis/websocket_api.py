...
class PoloniexSocketOperator:
    def __init__(
        self, connection_type: str, request_data: dict, send_kafka: bool, kafka_config: dict
    ):
        # #Display debug log
        ...
        try:
            if connection_type == "public":
                self.wsapp = websocket.WebSocketApp(
                    public_uri, on_message=on_message, on_pong=on_pong, on_open=on_open
                )
            else:
                self.wsapp = websocket.WebSocketApp(
                    private_uri, on_message=on_message, on_pong=on_pong, on_open=on_open
                )
        except:
        ...
        if send_kafka:
            ...
            self.kafka_producer = KafkaProducer(
                self.curr_date, self.curr_timestamp, self.producer_id
            )
